...
class ExplainAnomaliesUsingSHAP:
    ...
    def explain_unsupervised_data(self, x_train, x_explain, autoencoder=None):
        """
        ...
        Args:
            x_train (data frame): The data to train the autoencoder model on and to select the background set from (for SHAP
                                  explanation process)
            x_explain (data frame): The data from which the top 'num_anomalies_to_explain' records are selected by their MSE 
                                    to be explained.
            autoencoder (model): trained or untrained Autoencoder model that will be trained on given x_train data and
                                 then used to explain x_explain data. If None (model not provided) then we will build
                                 and train from scratch a Autoencoder model as described in train_model function.
        ...
        Returns:
            dict: Return all_sets_explaining_features dictionary that contain the explanation for
                  ...
                  lists; the explanation features sets. for example, for record index 24, with explanation features sets
                  ['F1', 'F2', 'F3'], all_sets_explaining_features will contain {24: ['F1', 'F2', 'F3']}
        ...
        for record_idx in top_records_to_explain:
            ...
            for idx_explained_feature in range(num_of_features):
                set_explaining_features = [x[1] for x in highest_contributing_features.index if
                                           x[0] == idx_explained_feature]
                explained_feature_index = df_top_err.index[idx_explained_feature]
                set_explaining_features.insert(0, x_train.columns[explained_feature_index])

                all_sets_explaining_features[record_idx].append(set_explaining_features)

            all_sets_explaining_features[record_idx] = list(
                dict.fromkeys(sum(all_sets_explaining_features[record_idx], [])))
